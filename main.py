...
class TimeButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        c.execute("""
        SELECT * FROM votes
        WHERE user_id=? AND day=? AND time_slot=?
        ...
        if exists:
            ...
            """, (user_id, user_name, self.day, self.slot))
...
class PollView(discord.ui.View):
    def __init__(self, day):
        ...
        for slot in TIME_SLOTS:
            self.add_item(TimeButton(day, slot))
...
@bot.tree.command(name="create_poll", description="4日分アンケート作成")
async def create_poll(interaction: discord.Interaction):
    ...
    for i in range(4):
        ...
        deadline_text = deadlines[i].strftime("〆%m/%d 18時")

        embed = discord.Embed(
            title=f"{DAY_TITLES[i]} {deadline_text}",
            color=0xf4c7ab
        )
        ...
        c.execute("""
        INSERT INTO polls (message_id, day, deadline)
        VALUES (?, ?, ?)
        """, (
            str(msg.id),
            i + 1,
...
@bot.tree.command(name="export_excel", description="Excel出力")
async def export_excel(interaction: discord.Interaction):
    ...
    for day in range(1, 5):
        ...
        if day == 1:
            ...
            ws = wb.create_sheet(title=DAY_TITLES[day - 1])

        ws.cell(row=1, column=1, value="名前")

        for i, slot in enumerate(TIME_SLOTS):
            ws.cell(row=1, column=i + 2, value=slot)

        for row_idx, user in enumerate(users, start=2):

            name_cell = ws.cell(row=row_idx, column=1, value=user)
        ...
        for col in range(1, len(TIME_SLOTS) + 2):
